...
const sessionAdvisor = require('./services/session-advisor');
...
// Burnout Analysis
ipcMain.handle('get-burnout-analysis', async (event, useDemoData = false, scenarioIndex = 0) => {
  try {
    const analysis = await burnoutAnalyzer.analyzeBurnoutRisk(useDemoData, scenarioIndex);
    return analysis;
  } catch (error) {
    console.error('[Main] Error getting burnout analysis:', error);
    return {
      status: 'error',
...
ipcMain.handle('get-weekly-burnout-heatmap', () => {
  return queries.getWeeklyBurnoutHeatmap();
});

// Session Duration Suggestions
ipcMain.handle('get-suggested-duration', async () => {
  try {
    const suggestion = await sessionAdvisor.getSuggestedDuration();
    return suggestion;
  } catch (error) {
    console.error('[Main] Error getting session suggestion:', error);
    return sessionAdvisor.getDefaultSuggestion();
  }
});
...
ipcMain.handle('get-system-time', () => {
  try {
    return sessionAdvisor.getAccurateSystemTime();
  } catch (error) {
    console.error('[Main] Error getting system time:', error);
    const now = new Date();
    return {
