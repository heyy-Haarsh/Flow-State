...
import { Play, Pause, Square, Timer, Lightbulb, Clock } from 'lucide-react';
...
export default function FocusTimer() {
  ...
  const [systemTime, setSystemTime] = useState<any>(null);

  // Load active session, tasks, and smart suggestions on mount
  useEffect(() => {
    loadActiveSession();
    ...
    // Load smart duration suggestion
    if (window.electron?.getSuggestedDuration) {
      window.electron.getSuggestedDuration().then((data: any) => {
        setSuggestion(data);
        // Auto-select suggested duration
        if (data && data.suggestedDuration) {
          setSelectedDuration(data.suggestedDuration * 60);
        }
      }).catch((err: any) => {
        console.error('[FocusTimer] Failed to load suggestion:', err);
      });
    }

    // Load accurate system time
    if (window.electron?.getSystemTime) {
      window.electron.getSystemTime().then((time: any) => {
        setSystemTime(time);
      }).catch((err: any) => {
        console.error('[FocusTimer] Failed to load system time:', err);
      });
    }
  }, []);
        ...
        <div className="flex flex-col items-center py-8">
          ...
          {!activeSession ? (
            <div className="w-full max-w-md space-y-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-sm font-semibold text-blue-300">
                          Recommended: {suggestion.suggestedDuration} min
                        </p>
                        {systemTime && (
                      ...
                      {suggestion.stats && (
                        <p className="text-xs text-blue-400/60 mt-1">
                          Based on {suggestion.stats.totalSessions} past sessions
                          {suggestion.stats.completionRate > 0 &&
                            ` (${suggestion.stats.completionRate}% completion rate)`
                          }
                        </p>
                      )}
                    </div>
              ...
              <div>
                <label className="block text-sm text-dark-300 mb-2">
                  Duration {suggestion && <span className="text-xs text-dark-500">(or choose manually)</span>}
                </label>
                <div className="grid grid-cols-4 gap-2">
                  {DURATION_PRESETS.map((preset) => (
                    ...
                    >
                      {preset.label}
                      {suggestion && preset.value === suggestion.suggestedDuration * 60 && (
